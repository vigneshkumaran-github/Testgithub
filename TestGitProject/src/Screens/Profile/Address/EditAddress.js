...
const EditAddress = ({route}) => {
  ...
  const navigation = useNavigation();
    const { language, UpdateAddress } = useContext(AuthContext);
  ...
  const reqobj = {
    ...
    useEffect(() => {
        setFlatNo(data?.house_number);
        setFloor(data?.house_floor);
        setLandMark(data?.landmark);
        setFullAdd(data?.full_address);
        setSelected(data?.type);
    }, [])

  const apicall = async () => {
    const result = await UpdateAddress(reqobj);
  ...
  return (
    <ScrollView style={styles.container}>
      <CustomHeader
        lable={language === 'English' ? 'Edit Address' : ' إضافة عنوان جديد'}
      ...
      <View style={{width: '90%', alignSelf: 'center', marginTop: 30}}>
        <CustomButton
          lable={language === 'English' ? 'Update Location' : 'تحديث الموقع'}
          onPress={() => {
            apicall();
          }}
        />
